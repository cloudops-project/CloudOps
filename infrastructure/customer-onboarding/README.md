# Customer Onboarding Templates â€” Stage 0 Placeholder
...
Templates will create a least-privilege read-only scan role trusting the exact CloudFix principal with a per-connection external ID. A distinct action-specific role/model will be required for approved remediation. Templates must never request customer access keys.
